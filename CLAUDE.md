...
│                       #  Pulse / Sparkle / Lightning / Aurora / Rocket.
...
| `Pixaroma.RunButton.FX` | combo | `js/run_button_fx/index.js` | Visual effect for ComfyUI's Run button. Options: None / Pixaroma Orange / Ignition / Pulse / Sparkle / Lightning / Aurora / Rocket. Default None - zero perf cost when disabled (MutationObserver never starts). Pure visual layer, never blocks queueing. |
...
11. **HTML5 drag-and-drop on rows with interactive children: ALWAYS gate `dragstart` so text selection inside child `<input>` / `<textarea>` works.** When a row has `draggable=true` AND contains a textarea or input the user types into (Prompt Stack rows, Prompt Multi rows, layer panel rename input, any future similar UI), drag-selecting text inside that field will hijack the row's HTML5 drag - the browser shows a ghost of the entire row following the cursor and text selection silently fails. Two valid patterns depending on the design:
    - **Preferred when the row has a dedicated drag handle (the ⋮⋮ icon)**: only allow drag from the handle. Strict check at the top of `dragstart`: `if (!e.target.closest(".pix-XX-handle")) { e.preventDefault(); return; }`. Reference: `js/prompt_multi/interaction.mjs::attachDragHandlers` + `js/prompt_stack/interaction.mjs::attachDragHandlers`. This is the most user-friendly because no edge case can hijack drag - only deliberate handle grab does.
