...
9. **`_TEXT_KEYS` / `_COND_LINK_KEYS` are `frozenset`s, not tuples.** The workflow walker does `key in _TEXT_KEYS` per node-input per extract, and tuples are O(n) linear scan. Frozenset gives O(1) membership and is the obvious clean version. If you add a new prompt-bearing widget name (e.g. some new node's `wildcard_text_extra` field), add it to the set in `nodes/_prompt_reader_helpers.py` — the walker picks it up automatically.
