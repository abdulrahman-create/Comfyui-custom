...
2. **Schema migration via version field.** `DEFAULT_STATE.version = 2`. `ensureValidState(node)` in `index.js` resets state to defaults when `state.version !== 2`. v1 multi-layer state (which had `layers: [...]`) is detected by the missing/wrong version and dropped, no migration of individual fields. This is acceptable because v1 was never publicly released.

3. **Bg pill is a single color field, not a nested object.** `state.bgColor` is either `null` (no pill) or a hex string. Padding (16/10) and radius (6) are hardcoded in both renderers. If a future user request needs per-property control, swap `bgColor: "#xxx"` → `bgPill: { color, paddingX, paddingY, radius }`. Until then, simpler.
...
8. **DOM widget `getMinHeight` sums children offsetHeight.** Don't use `root.scrollHeight`, same trap as Load Image Pixaroma Pattern #4. The DOM widget root gets stretched by LiteGraph when the node has slack height; `scrollHeight` reads the stretched value and creates a feedback loop. Iterate children and sum their `offsetHeight` instead. Cap with `Math.max(380, h)` so the node has a sensible minimum when collapsed.

9. **Default node size lives in `setupTextOverlayNode` AFTER `addDOMWidget`.** Set `node.size = [360, 700]` directly. LiteGraph's `configure` runs AFTER `nodeCreated` and overwrites `node.size` from saved workflow JSON, so existing workflows keep their saved size while fresh-on-canvas drops get the new default.
