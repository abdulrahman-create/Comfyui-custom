...
- **Still in progress:** the Switch family (Switch, Switch Source, Mute Switch) lines its controls up with the connection dots, which the new interface handles very differently, so for now those work best in the Classic interface. The Version Check node shows you at a glance which interface you're in.
