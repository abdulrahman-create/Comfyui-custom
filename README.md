...
### 🎚️ Sliders Pixaroma
One panel that holds every number you keep reaching for. Add a slider, wire it to any number input in the workflow - steps, cfg, denoise, a LoRA strength - and from then on you tweak it from the panel instead of hunting through the graph. Drag across a slider to set it, hold **Shift** while dragging for fine control, or double-click to type an exact value. Each slider carries its own output, sitting on its own row. A new slider is set to **Auto**, and the first input you connect it to teaches it everything it needs: whether to send a whole number or a decimal, plus a sensible name, range and step - and it adopts the value that input is already on, so connecting never changes your workflow behind your back. Because each slider takes on its input's type, it cannot send the wrong kind of number. **Right-click** the node for the settings, where you add and remove sliders, rename them, set each range, and pick the colour the sliders paint with (per node, and you can save your colour as the default for every new panel). Up to 16 sliders per node, and you can add as many panels as you like. Works in both the classic and the new node interface.
