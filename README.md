...
### **May 19, 2026 (1.3.44)**
- **Prompt Pack: counter no longer sticks at "1 left" after a fast run.** When the downstream workflow was very simple (Prompt Pack → Text Pixaroma → nothing), the workflow could finish executing on the backend before the queue submission response came back, and the "X left" counter would miss that finish event and stay stuck. Fixed: the counter now always counts down cleanly to the total prompt count, no matter how trivial or how fast the run is.
- **Text Pixaroma: typing-only, with a clearer wire story.** Text Pixaroma's textarea is for typing into directly. The Vue frontend was exposing that textarea as a wireable input anyway, which made it look broken when you wired Prompt Pack (or anything else) into it and the typed text didn't visually update. Now: new wires refuse to connect to Text Pixaroma with a clear toast telling you to use **Show Text Pixaroma** instead (which is the right node for receiving text from another node). Workflows you already saved with a wire into Text Pixaroma still load and still run correctly; the textarea now goes gray and a small orange hint reads *"Wired from upstream; typing here is ignored"* so you can see what's actually happening at a glance, and the Replace and Clear buttons disable since the typed value is overridden anyway.
