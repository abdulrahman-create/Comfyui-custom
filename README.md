...
### **May 12, 2026 (1.3.22)**
- **Preview Image Pixaroma - Copy + Open buttons:** Two new orange buttons next to Save Disk / Save Output. **Copy** writes the selected frame's PNG to your OS clipboard via the standard `navigator.clipboard.write([new ClipboardItem(...)])` API (works in Chrome / Edge / Firefox 127+ / Safari 16+); paste straight into another node, paint app, message, etc. **Open** opens the selected frame in a new browser tab for full-screen viewing or comparing multiple side by side. Both honour the currently selected frame in batches, so you get the frame you clicked, not always the first or last. Also fixed: dragging the node smaller than the 4-button row used to let buttons overflow past the node frame because Vue's `onResize` and Align Pixaroma's resize intercept can both bypass the clamping; size enforcement now self-heals at draw time.
- **Drag-and-drop on Image Crop / Image Composer / Paint:** Drop an image file anywhere on the closed node body. **Image Crop** silently sets the dropped image as the source and auto-disconnects any upstream wire (same semantics as the existing Ctrl+V paste flow). **Image Composer** and **Paint** open the editor and add the dropped image as a new layer on top of any existing layers (never replaces, mirrors Photoshop / Figma / Affinity behavior); the editor waits for restore to finish before stacking the new layer so the order is predictable.
- **Image Crop - Load Image button + center-crop fix:** The in-editor `Load Image` button no longer hides itself when an upstream is wired - it now auto-disconnects the wire when used (consistent with paste / drop). Also fixed a long-standing center-crop bug: changing W or H in the on-node panel right after loading a workflow used to leave X / Y at 0 even with `Center Crop` selected, producing a top-left crop instead of a centered one. The panel now persists `original_w/h` into the saved JSON whenever the upstream image loads, and falls back to that when the runtime image-dims field is lost across Vue tab switches.
- **Brand defaults across every Pixaroma node:** All 14 nodes (Paint, 3D Builder, Image Composer, Image Crop, Note, Preview Image, Notify, Load Image, Resolution, Show Text, Save Mp4, Image Compare, AudioReact, Reference) now spawn with the same dark `#1d1d1d` title bar / `#2a2a2a` body so the suite looks like one cohesive brand. Single global extension under `js/brand/index.js` hooks every node whose Python `CATEGORY` starts with `👑 Pixaroma`; future nodes inherit automatically. Right-click → Colors and saved workflow colors still win (guarded by `if (!node.color)`).
- **Load Image Pixaroma - drop UX cleanup:** Drop anywhere on the closed node body (no orange "Drop to upload" overlay anymore). The overlay implied the orange controls panel was the only valid drop target when the entire node accepts drops via ComfyUI's native handler. Wrapped `imageWidget.callback` so the dims info bar refreshes correctly even when you drop on the bottom native preview area instead of the controls panel.
- **Server: defensive `mkdir` per upload request:** All routes that save images under `input/pixaroma/` now `os.makedirs(...)` per request inside `_safe_path`. The previous module-load `os.makedirs(..., exist_ok=True)` ran once at server startup and a stale `extra_model_paths.yaml` or a manual `input/` cleanup while ComfyUI was running could leave the directory missing, causing the next save to 500 with `FileNotFoundError`. Now self-heals.
