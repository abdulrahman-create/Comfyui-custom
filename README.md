...
- **Remove Background Pixaroma: model dropdown built into the node, with HR and matting support.** No more separate Load Background Removal Model node to wire in. Pick your model from a dropdown right on the node. Three variants supported, each tuned for a different use case:
    ...
    - **High Resolution** (444 MB, 8 GB+ VRAM, processes at 2048×2048): large images where fine outline detail matters - jewelry, intricate hardware, complex shapes.
    - **Soft Alpha Edges / Matting** (444 MB, 8 GB+ VRAM, processes at 2048×2048): hair, fur, lace, soft fabric. Also worth trying for glass, smoke, sheer materials.
- Drop the `.safetensors` files into `ComfyUI/models/background_removal/`. If the folder is empty the node tells you exactly what to download and where to put it. The right processing resolution is picked automatically from the filename so you do not have to think about it. Your output image stays the same size as your input regardless of which model you pick.
- Heads-up: workflows saved with the older version of the node lose the input cable on load - just pick your model from the new dropdown and re-save.
