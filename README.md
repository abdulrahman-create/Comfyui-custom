<div align="center">
  ...
  <p align="center">
    ...
    Load Image • Crop • Compose • Paint • 3D • Compare • Preview • Save MP4 • Notes & Labels • Resolution
  </p>
...
- **Brand defaults across every Pixaroma node:** All 14 nodes (Paint, 3D Builder, Image Composer, Image Crop, Note, Preview Image, Notify, Load Image, Resolution, Show Text, Save Mp4, Image Compare, AudioReact, Reference) now open with the same dark title bar and body so the suite looks like one cohesive brand. Right-click → Colors and saved workflow colors still win, so you can customize per node.
- **Load Image Pixaroma - drop UX cleanup:** Drop anywhere on the node (the orange "Drop to upload" overlay is gone). The dimensions info bar also updates correctly when you drop on the bottom preview area.
- **Server self-heals if the upload folder is missing:** All upload routes now re-create the upload folder on the fly if it got deleted or moved while ComfyUI was running. Previously this caused upload errors until restart.
