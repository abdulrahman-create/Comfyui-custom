...
//
// Layout (top -> bottom):
//   - Header: Active: < N / Total > picker + label preview of the picked row
//   - Library: one row per labeled prompt (small idx + label + textarea +
//     drag handle + X). Active row gets an orange left border.
//   - Actions: + Add prompt | Clear text | Reset
//
// Two outputs are wired on the Python side (text + prompts). The active
// picker UI controls which row's text goes to the `text` output. The full
// list (`prompts` output) just emits every row's text in display order.
//
...
export function renderRows(node, root, rowHandlers) {
  ...
  indexInput.title = "Active row number (1-based) - drives the `text` output";
  ...
  state.rows.forEach((row, idx) => {
    ...
    rowIdx.title = "Click to make this the active row (drives the `text` output)";
    ...
    ta.value = row.text || "";
    ta.rows = 2;
    ta.placeholder = "Type a prompt. The active row's text goes to `text`; every row goes to `prompts` for the list pattern.";
