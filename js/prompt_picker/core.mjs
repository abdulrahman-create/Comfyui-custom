...
// Shape: { version: 3, rows: [ { id, label, text } ], activeIndex }
//
...
// active row's text AND the full list of row texts into the hidden
// PromptPickerState input at workflow-submit time:
//   { version, activeText, rowTexts: [string, ...] }
...
export function readState(node) {
  ...
  // Migration: v2 schema had a picks[] array (the brief multi-output era).
  // Map picks[0].rowIndex back to a single activeIndex if present.
  if (Array.isArray(s.picks)) {
    const firstPick = s.picks[0];
    if (firstPick && typeof firstPick.rowIndex === "number") {
      s.activeIndex = firstPick.rowIndex;
    }
    delete s.picks;
  }
